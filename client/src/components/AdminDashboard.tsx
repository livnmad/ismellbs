...
const AdminDashboard: React.FC = () => {
  ...
  const fetchAppUsers = async () => {
    ...
    try {
      const response = await axios.get('/api/admin/app-users', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteAdmin = async (username: string) => {
    ...
    const token = localStorage.getItem('adminToken');
    try {
      const response = await axios.delete(`/api/admin/users/${username}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleToggleUserStatus = async (userId: string, isActive: boolean) => {
    ...
    try {
      const response = await axios.post(
        `/api/admin/app-users/${userId}/toggle`,
        { isActive },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDeleteAppUser = async (userId: string) => {
    ...
    const token = localStorage.getItem('adminToken');
    try {
      const response = await axios.delete(`/api/admin/app-users/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
