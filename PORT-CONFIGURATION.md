...
**Single Port Architecture**: Everything runs through port 3000

```
Port 3000 → Backend Express Server
            ├── /api/* → API endpoints
            └── /*     → Static React build files
```

### What runs where:
- **Backend Server**: Port 3000 (inside and outside container)
- **Elasticsearch**: Port 9200 (inside Docker network)
- **React Build**: Served as static files from backend on port 3000
...
- With Nginx: `http://your-domain.com` (Nginx proxies to :3000)
...
```
Port 3000 → Backend (serves API + static build)
```
...
1. **Production**: Only port 3000 is exposed. Backend serves everything.
2. **Development**: Port 3000 for React dev server, 3000 for API (optional)
...
- **3000** (App) - if accessing directly
