...
const API_URL = 'https://ismellbullshit.com/api';
...
async function createPost(post: any) {
  try {
    ...
    console.log(`✓ Created post by ${post.author} (${post.daysAgo} days ago)`);
...
async function createComment(postId: string, author: string, content: string) {
  try {
...
async function seedData() {
  ...
  for (const post of posts) {
    ...
    if (postId) {
      ...
      const numComments = Math.random() > 0.5 ? 4 : 3;
      const selectedComments = getRandomItems(comments, numComments);
      
      for (let i = 0; i < numComments; i++) {
        // 50% chance of anonymous comment
        const isAnonymous = Math.random() > 0.5;
        const commentAuthor = isAnonymous ? 'Anonymous' : getRandomItem(names);
        
        await createComment(postId, commentAuthor, selectedComments[i]);
